...
The visual preserves Chapter 28’s frozen path: **Researcher → Front door → admission/ticket boundary → identity badge check → Coordinator Runtime → bounded services / Records**. Model, retrieval, and tool gateways remain ordinary bounded services. Specialist delegation is a Runtime-coordinator handoff to a separate specialist runtime with its own bounded policy and allowlisted tools, not a tool call, a nested drawing under the toolbox, or an expansion of authority. Technical labels use the vendor-neutral catalog: Experience adapter, API and admission control, Runtime, Policy decision point, Model gateway, Retrieval service, Connector and tool gateway, Approval service, State and artifact stores, and Observability pipeline.
